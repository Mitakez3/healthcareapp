...
class _ActivityScreenState extends State<ActivityScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Vận động", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black)),
      ...
      body: StreamBuilder<DatabaseEvent>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasData && snapshot.data!.snapshot.value != null) {
              ...
              bool isToday = false;
              if (lastUpdated > 0) {
                final DateTime date = DateTime.fromMillisecondsSinceEpoch(lastUpdated);
                final DateTime now = DateTime.now();
              ...
              for (int i = 0; i < 7; i++) {
                ...
                if (weekDays[i] == "Th 2" || weekDays[i] == "Mon") weekDays[i] = "T2";
                if (weekDays[i] == "Th 3" || weekDays[i] == "Tue") weekDays[i] = "T3";
                if (weekDays[i] == "Th 4" || weekDays[i] == "Wed") weekDays[i] = "T4";
                if (weekDays[i] == "Th 5" || weekDays[i] == "Thu") weekDays[i] = "T5";
                if (weekDays[i] == "Th 6" || weekDays[i] == "Fri") weekDays[i] = "T6";
                if (weekDays[i] == "Th 7" || weekDays[i] == "Sat") weekDays[i] = "T7";
                if (weekDays[i] == "CN" || weekDays[i] == "Sun") weekDays[i] = "CN";
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(child: _buildStatCard("Calorie", "$calories", "kcal", FontAwesomeIcons.fire, Colors.orange)),
                  ...
                  if (historyList.isEmpty)
                    Container(
                      ...
                      child: const Center(child: Text("Chưa có lịch sử hoạt động", style: TextStyle(color: Colors.grey))),
                    )
                  else
                    Column(
    ...
    return BarChartGroupData(
        ...
        barRods: [
          BarChartRodData(
              ...
              backDrawRodData: BackgroundBarChartRodData(show: true, toY: backgroundMax, color: Colors.grey.shade100)
          )
        ]
    );
  }
  ...
  Widget _buildStatCard(String title, String value, String unit, IconData icon, Color color) {
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(15)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon, color: color, size: 24), const SizedBox(height: 10), Text(value, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), Text("$unit • $title", style: const TextStyle(fontSize: 12, color: Colors.grey))]));
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 15),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)), const SizedBox(height: 4), Text(time, style: TextStyle(color: Colors.grey.shade500, fontSize: 13))])),
